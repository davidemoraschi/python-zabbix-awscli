...
args: Dict[str, str] = process_arguments(options=['WORKFLOW_NAME', 'WORKFLOW_RUN_ID', 'SKILL_REPORT_IDS'])
...
SKILL_REPORT_IDS: List[str] = json.loads(args['SKILL_REPORT_IDS'])
...
def create_export_report_csv(report_id: str) -> str:
    ...
    try:
        ...
        # Construct API URL
        url: str = f'https://skill.straumann.com/api/crm/v1/reportdata/{report_id}'
        ...
        report_payload: str = response.json()#['data']['executionId']
        ...
        for report_id in SKILL_REPORT_IDS:
            ...
            for jsonfile in filelist:
                ...
                upload_json_gz(s3client=s3client, bucket=S3_BUCKET,
                               key=S3_PATH + f'{str(JOB_RUN_ID)}-{s3_filename}.gz', obj=json.dumps(report_payload))
