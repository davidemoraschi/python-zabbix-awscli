...
locals {
  ...
  sapcdc_api_secret_name   = "stg/dlk/sapcdc/ds9/secrets"
}
...
resource "aws_iam_policy" "raw_job_role_policy" {
  ...
  policy = templatefile("${path.module}/artifacts/ds${var.datasource_number}/glue/policies/glue-role-policy.json", {  
    ...
    sapcdc_api_secret_name  = local.sapcdc_api_secret_name
...
resource "aws_s3_object" "raw_glue_job_script" {
  bucket      = local.artifacts_bucket_name
  key         = "artifacts/glue_job_${local.datasource}/code/get_audit_table.py"
  source      = "${path.module}/artifacts/ds${var.datasource_number}/glue/code/get_audit_table.py"
  source_hash = filemd5("${path.module}/artifacts/ds${var.datasource_number}/glue/code/get_audit_table.py")
}
...
resource "aws_glue_job" "raw_glue_job" {
  ...
  command {
    name                          = "pythonshell"
    script_location               = "s3://${local.artifacts_bucket_name}/artifacts/glue_job_${local.datasource}/code/get_audit_table.py"
    python_version                = 3.9
  }
