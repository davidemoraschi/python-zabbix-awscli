...
# 1. Does a POST call to SAP CDC API to get the accounts
# 2. Does a POST call to SAP CDC API to get the audit
# 3. UpPloads them to S3 bucket
...
from common_functions import get_last_loaded_epoch, log, process_arguments, log_environment, create_etl_log_table, send_sns
...
def main():
    ...
    try:
        ...
        # Calls the API to get the auditlog
        ...
        log(workflow_name=WORKFLOW_NAME, workflow_run_id=WORKFLOW_RUN_ID, job_name=JOB_NAME,
            job_run_id=str(JOB_RUN_ID), str_message=f'latest_epoch:{latest_epoch}, last_loaded_epoch:{last_loaded_epoch}')
        ...
        while True:
            ...
            # Log pagination
            log(workflow_name=WORKFLOW_NAME, workflow_run_id=WORKFLOW_RUN_ID, job_name=JOB_NAME,
                job_run_id=str(JOB_RUN_ID), str_message=f'{objectscount} of {totalCount} rows')
        ...
        # Log latest loaded epoch
        log(workflow_name=WORKFLOW_NAME, workflow_run_id=WORKFLOW_RUN_ID, job_name=JOB_NAME,
            job_run_id=str(JOB_RUN_ID), str_message='Latest loaded Epoch', str_error_message=latest_epoch)

        # Log succesful end of job
        log(workflow_name=WORKFLOW_NAME, workflow_run_id=WORKFLOW_RUN_ID, job_name=JOB_NAME,
            job_run_id=str(JOB_RUN_ID), str_message='end-job', str_error_message=f'Success.')
        ...
        log(workflow_name=WORKFLOW_NAME, workflow_run_id=WORKFLOW_RUN_ID, job_name=JOB_NAME,
            job_run_id=str(JOB_RUN_ID), str_message='end-job', str_error_message=f'Failure.')
